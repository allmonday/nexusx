...
| Phase | 职责 | 产出 |
|-------|------|------|
| **Phase 1** | Schema + ER Diagram + 聚合根入口 + mock seed | models + database(engine + seed) + voyager |
...
├── database.py     # Phase 1（engine + mock seed）
...
**新增/修改文件**:
- `models.py` — SQLModel 实体 + Relationship + `@query`/`@mutation`（方法体 `pass` + docstring）
- `database.py` — aiosqlite engine + session_factory + mock seed data（供团队讨论数据样本）
...
3. **列表关系需要 order_by** — 分页功能要求 `sa_relationship_kwargs={"order_by": "Entity.column"}`
4. **ErManager base 和 entities 互斥** — 不能同时提供
5. **目录命名不能以数字开头** — Python 模块名限制
6. **UseCaseService 只有被 @query/@mutation 装饰的 async classmethod 会被发现** — 普通方法不会暴露
7. **build_dto_select → dict(row._mapping) → DTO 构造** — 这是 Core API 的标准查询模式
8. **每个 Model 必须有 docstring，每个 Field 必须有 description** — Phase 1 就要确保语义清晰，description 会传递到 OpenAPI spec
9. **每个 service 子目录必须包含 spec.md** — 记录服务目的、用途、方法需求、DTO 说明和变更记录，方便团队理解服务边界
...
3. **Phase 1**: 生成 models.py + database.py(mock seed) + main.py(voyager) → 展示 ER diagram + SDL + seed data → **暂停等用户确认**
